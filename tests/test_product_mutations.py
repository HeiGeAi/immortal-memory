...
def seed_real_claim(vault, claim_id="clm_governance"):
    ...
    created = claims.create(
        value,
        expected_revision=0,
        request_id="seed-" + claim_id,
        idempotency_key="seed-" + claim_id,
...
def test_claim_reconsider_action_delegates_to_claim_store(tmp_path):
    service, claims, _living = coordinator(tmp_path)
    body = {
        "action": "reconsider",
        "expected_version": 2,
    ...
    result = service.mutate(
        "/api/v2/claims/clm_1/actions",
        body,
        **metadata("claim-reconsider"),
    )

    assert result["status"] == "candidate"
    assert claims.calls[0][0:2] == ("reconsider", "clm_1")
    assert claims.calls[0][2]["evidence_ids"] == ["ev_new"]
    assert claims.calls[0][2]["expected_revision"] == 2


def test_claim_correct_action_delegates_to_claim_store(tmp_path):
    service, claims, _living = coordinator(tmp_path)
    body = {
        "action": "correct",
        "expected_version": 3,
    ...
    result = service.mutate(
        "/api/v2/claims/clm_1/actions",
        body,
        **metadata("claim-correct"),
    )

    assert result["status"] == "confirmed"
    assert claims.calls[0][0:3] == ("correct", "clm_1", "corrected statement")
    assert claims.calls[0][3]["expected_revision"] == 3
...
def test_claim_governance_actions_require_action_specific_fields(tmp_path, body):
    service, _claims, _living = coordinator(tmp_path)

    with pytest.raises(MutationError) as caught:
        service.mutate(
            "/api/v2/claims/clm_1/actions",
            body,
            **metadata("claim-fields-" + body["action"]),
        )
...
def test_missing_claim_preserves_claim_not_found_code(tmp_path):
    vault = tmp_path / "vault"
    service = ProductMutationCoordinator(
        vault,
        claims=ClaimStore(vault),
        living_self=LivingSelfService(
            vault, clock=lambda: "2099-01-01T00:00:00+00:00"
        ),
        judgments=object(),
        compiler=object(),
        outcomes=object(),
    ...
    with pytest.raises(MutationError) as caught:
        service.mutate(
            "/api/v2/claims/clm_missing/actions",
            {"action": "confirm", "expected_version": 1, "reason": "owner review"},
            **metadata("missing-claim"),
        )

    assert caught.value.code == "claim_not_found"


def test_real_reconsider_requires_evidence_not_already_on_claim(tmp_path):
    vault = tmp_path / "vault"
    claims, created = seed_real_claim(vault, "clm_reconsider_existing")
    rejected = claims.transition(
        created["claim_id"],
        "rejected",
        expected_revision=created["revision"],
        request_id="seed-reject-existing",
        idempotency_key="seed-reject-existing",
        actor={"kind": "owner", "id": "owner"},
        reason="seed rejected claim",
    )
    service = ProductMutationCoordinator(
        vault,
        claims=claims,
    ...
    with pytest.raises(MutationError) as caught:
        service.mutate(
            "/api/v2/claims/clm_reconsider_existing/actions",
            {
                "action": "reconsider",
                "expected_version": rejected["revision"],
                "reason": "same evidence is insufficient",
                "evidence_ids": ["ev_original"],
            },
            **metadata("reconsider-existing"),
        )

    assert caught.value.code == "new_evidence_required"


def test_real_reconsider_is_idempotent_and_readable_after_restart(tmp_path):
    vault = tmp_path / "vault"
    claims, created = seed_real_claim(vault, "clm_reconsider_restart")
    rejected = claims.transition(
        created["claim_id"],
        "rejected",
        expected_revision=created["revision"],
        request_id="seed-reject-restart",
        idempotency_key="seed-reject-restart",
        actor={"kind": "owner", "id": "owner"},
        reason="seed rejected claim",
    )
    body = {
    ...
    first = ProductMutationCoordinator(
        vault,
        claims=claims,
        living_self=LivingSelfService(
            vault, clock=lambda: "2026-07-22T00:00:00+00:00"
        ...
        "/api/v2/claims/clm_reconsider_restart/actions",
        body,
        **metadata("reconsider-restart"),
    )
    retry = ProductMutationCoordinator(
        vault,
        claims=ClaimStore(vault),
    ...
    current = ClaimStore(vault).get("clm_reconsider_restart")
    assert retry == first
    assert current["status"] == "candidate"
    assert current["evidence_ids"] == ["ev_original", "ev_new"]
    assert current["revision"] == rejected["revision"] + 1


def test_real_reconsider_preserves_version_conflict(tmp_path):
    vault = tmp_path / "vault"
    claims, created = seed_real_claim(vault, "clm_reconsider_stale")
    rejected = claims.transition(
        created["claim_id"],
        "rejected",
        expected_revision=created["revision"],
        request_id="seed-reject-stale",
        idempotency_key="seed-reject-stale",
        actor={"kind": "owner", "id": "owner"},
        reason="seed rejected claim",
    )
    service = ProductMutationCoordinator(
    ...
    with pytest.raises(MutationError) as caught:
        service.mutate(
            "/api/v2/claims/clm_reconsider_stale/actions",
            {
                "action": "reconsider",
                "expected_version": rejected["revision"] - 1,
                "reason": "stale review",
                "evidence_ids": ["ev_new"],
            },
            **metadata("reconsider-stale"),
        )

    assert caught.value.code == "version_conflict"


def test_real_correct_is_idempotent_and_readable_after_restart(tmp_path):
    vault = tmp_path / "vault"
    claims, created = seed_real_claim(vault, "clm_correct_restart")
    confirmed = claims.transition(
        created["claim_id"],
        "confirmed",
        expected_revision=created["revision"],
        request_id="seed-confirm-correct",
        idempotency_key="seed-confirm-correct",
        actor={"kind": "owner", "id": "owner"},
        reason="seed confirmed claim",
    )
    living = LivingSelfService(vault, clock=lambda: "2099-01-01T00:00:00+00:00")
    living.confirm(living.build_candidate(), reason="seed Living Self")
    body = {
        "action": "correct",
    ...
    first = ProductMutationCoordinator(
        ...
        "/api/v2/claims/clm_correct_restart/actions",
        body,
        **metadata("correct-restart"),
    )
    retry = ProductMutationCoordinator(
        vault,
        claims=ClaimStore(vault),
    ...
    reopened_claims = ClaimStore(vault)
    original = reopened_claims.get("clm_correct_restart")
    replacement = reopened_claims.get(first["claim_id"])
    current_self = LivingSelfService(
        vault, clock=lambda: "2099-01-01T00:00:00+00:00"
    ).current()
    assert retry == first
    ...
    assert replacement["statement"] == "corrected statement"
    assert current_self["version_id"] == first["derived_version_id"]
    assert "corrected statement" in json.dumps(current_self, ensure_ascii=False)
