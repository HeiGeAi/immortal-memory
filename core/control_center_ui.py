...
:root {
  ...
  --radius-md: 10px;
  --radius-lg: 16px;
  --mono: "JetBrains Mono", "SF Mono", ui-monospace, "Noto Sans SC", "PingFang SC", monospace;
  --display: "Geist", "Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", "Noto Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif;
  --body: "Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", "Noto Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif;
...
.brand-mark {
  ...
  background: linear-gradient(145deg, rgba(45,212,191,.15), transparent 65%);
  box-shadow: 0 0 28px rgba(45,212,191,.10);
}
.brand-mark::before, .brand-mark::after { content: ""; position: absolute; background: var(--signal); }
.brand-mark::before { width: 1px; height: 14px; left: 15px; top: 8px; }
.brand-mark::after { width: 14px; height: 1px; left: 8px; top: 15px; }
.eyebrow, .micro, .tag, .mono { font-family: var(--mono); }
.eyebrow { color: var(--signal); font-size: 10px; letter-spacing: .18em; }
.brand h1 { margin: 2px 0 0; font: 600 19px/1 var(--display); letter-spacing: .02em; }
.system-ribbon {
  display: flex;
  align-items: center;
  gap: 9px;
...
.beacon {
  ...
  box-shadow: 0 0 0 4px rgba(89,98,110,.08);
  transform-origin: center;
}
.beacon.healthy, .beacon.success, .beacon.ready { background: var(--signal); border-color: var(--glow); }
.beacon.running { background: var(--glow); border-color: var(--signal); box-shadow: 0 0 0 4px rgba(45,212,191,.13), 0 0 16px rgba(45,212,191,.30); animation: beacon 2.4s ease-in-out infinite; }
...
.version { color: var(--muted); font: 11px var(--mono); margin-right: 4px; }
.nav {
  display: grid;
...
.nav-button:hover:not(:disabled) { color: var(--paper); background: rgba(255,255,255,.018); }
.nav-button.active { color: var(--paper); background: rgba(45,212,191,.035); }
.nav-button.active::after { transform: scaleX(1); }
.nav-button:disabled { color: var(--faint); cursor: not-allowed; }
.nav-button strong { display: block; font: 600 12px var(--display); letter-spacing: .02em; }
.nav-button small { color: var(--faint); font: 9px var(--mono); letter-spacing: .09em; }
.nav-button:focus-visible, .control:focus-visible, .field:focus-visible {
  outline: 2px solid var(--glow);
  outline-offset: 2px;
}
.view { min-height: 680px; padding-top: 32px; }
...
.view-kicker { color: var(--signal); font: 10px var(--mono); letter-spacing: .17em; }
.view-head h2 { margin: 6px 0 4px; font: 600 clamp(32px, 4vw, 52px)/1.08 var(--display); letter-spacing: -.045em; }
...
.hero::before {
  ...
  inset: -28%;
  pointer-events: none;
  background: radial-gradient(ellipse at 88% 6%, rgba(45,212,191,.18), rgba(45,212,191,.06) 25%, transparent 57%);
  transform: rotate(-8deg);
}
.hero-main, .hero-side { position: relative; z-index: 1; }
.hero-main { padding: clamp(32px, 5vw, 68px); }
.verdict { margin: 30px 0 14px; color: var(--glow); font: 600 clamp(54px, 7vw, 92px)/.92 var(--display); letter-spacing: -.065em; }
.verdict.attention, .verdict.partial, .verdict.stale { color: var(--warn); }
.verdict.failed, .verdict.error, .verdict.not_ready { color: var(--bad); }
...
.hero-stat span { color: var(--muted); font: 10px var(--mono); letter-spacing: .08em; }
.hero-stat strong { color: var(--signal); font: 600 clamp(30px, 2.8vw, 44px)/1 var(--mono); font-variant-numeric: tabular-nums; letter-spacing: -.07em; }
.section-title { display: flex; justify-content: space-between; align-items: end; gap: 20px; margin: 32px 0 12px; }
.section-title h3 { margin: 0; font: 600 20px var(--display); letter-spacing: -.01em; }
...
button.card:hover { border-color: var(--line-hot); background: var(--deck-2); }
.card h4 { margin: 16px 0 7px; font: 600 18px var(--display); letter-spacing: -.01em; overflow-wrap: anywhere; }
.card p { margin: 0; color: var(--muted); font-size: 13px; overflow-wrap: anywhere; }
.card .source { margin-top: 16px; color: var(--faint); font: 10px var(--mono); word-break: break-all; }
...
.tag.healthy, .tag.success, .tag.ready, .tag.running { color: var(--signal); border-color: rgba(45,212,191,.42); }
.tag.attention, .tag.partial, .tag.stale, .tag.skipped { color: var(--warn); border-color: rgba(246,163,92,.45); }
...
.metric strong { display: block; color: var(--signal); font: 600 clamp(34px, 5vw, 58px)/1 var(--mono); font-variant-numeric: tabular-nums; letter-spacing: -.07em; }
.metric span { display: block; margin-top: 18px; color: var(--muted); font-size: 12px; }
.toolbar {
  display: grid;
...
.field:hover, .field:focus { border-color: var(--line-hot); background: var(--deck); }
...
.control:hover:not(:disabled) { border-color: rgba(45,212,191,.55); background: var(--deck-2); }
.control:active:not(:disabled) { transform: scale(.98); }
.control.primary { border-color: var(--signal); background: var(--signal); color: var(--on-signal); box-shadow: 0 0 40px rgba(45,212,191,.24); }
.control.primary:hover:not(:disabled) { border-color: var(--glow); background: var(--glow); color: var(--on-signal); }
.control.danger { border-color: rgba(255,119,109,.45); color: var(--bad); }
.control.danger:hover:not(:disabled) { border-color: var(--bad); background: rgba(255,119,109,.08); }
.control:disabled { opacity: .48; cursor: not-allowed; transform: none; }
.control.is-loading { cursor: wait; opacity: .72; }
.control.is-success { border-color: var(--signal); color: var(--signal); }
.control.primary.is-success { background: var(--signal); color: var(--on-signal); }
.control.is-error { border-color: var(--bad); color: var(--bad); }
.control span { display: grid; gap: 2px; }
.control strong { display: block; font: 600 13px/1 var(--display); letter-spacing: .01em; }
.control small { display: block; color: var(--muted); font: 9px/1.1 var(--mono); letter-spacing: .08em; }
.control.primary small { color: rgba(3,24,21,.68); }
.row-list { overflow: hidden; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--deck); }
.row {
  display: grid;
...
button.row:hover { background: rgba(45,212,191,.035); }
.row strong { overflow-wrap: anywhere; }
.row p { margin: 0; color: var(--muted); font-size: 12px; overflow-wrap: anywhere; }
.row .mono { color: var(--faint); font-size: 10px; word-break: break-all; }
.stage-rail { display: grid; gap: 1px; overflow: hidden; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--line); }
.stage {
  display: grid;
...
.stage-index { display: grid; place-items: center; width: 34px; height: 34px; border: 1px solid var(--line); border-radius: 50%; color: var(--faint); font: 10px var(--mono); }
.stage.running .stage-index, .stage.success .stage-index { color: var(--signal); border-color: var(--signal); }
...
.stage time { color: var(--faint); font: 10px var(--mono); text-align: right; }
...
.error-box { color: var(--bad); border-color: rgba(255,119,109,.46); }
.skeleton { background: linear-gradient(110deg, var(--deck), var(--deck-2), var(--deck)); }
.pagination { display: flex; justify-content: space-between; gap: 10px; align-items: center; margin-top: 12px; color: var(--muted); }
.drawer {
  position: fixed;
  z-index: 30;
  top: 0;
...
.drawer-head { position: sticky; top: 0; display: flex; justify-content: space-between; gap: 12px; align-items: center; padding: 20px; border-bottom: 1px solid var(--line); background: var(--deck); }
.drawer-body { padding: 24px; }
.drawer-body h3 { font: 600 28px var(--display); }
.detail-block { margin: 14px 0; padding: 16px; border: 1px solid var(--line); border-radius: var(--radius-md); background: var(--void); }
.detail-block label { display: block; margin-bottom: 8px; color: var(--signal); font: 10px var(--mono); letter-spacing: .1em; }
.detail-block pre { margin: 0; white-space: pre-wrap; word-break: break-word; color: var(--paper); font: 11px/1.65 var(--mono); }
.dialog {
...
.dialog::backdrop { background: rgba(8,9,13,.78); }
.dialog-head { padding: 24px; border-bottom: 1px solid var(--line); }
.dialog-head .micro { color: var(--signal); font-size: 10px; letter-spacing: .12em; }
.dialog-head h3 { margin: 8px 0 0; font: 600 28px var(--display); }
.dialog-body { padding: 24px; color: var(--muted); }
.dialog-impact { margin: 16px 0 0; padding: 14px; border-left: 2px solid var(--warn); border-radius: var(--radius-sm); background: rgba(246,163,92,.06); color: var(--paper); }
.dialog-actions { display: flex; justify-content: flex-end; gap: 8px; padding: 16px 24px; border-top: 1px solid var(--line); }
.toast {
  position: fixed;
  z-index: 50;
  right: 24px;
...
.footer { display: flex; justify-content: space-between; gap: 16px; margin-top: 40px; padding-top: 18px; border-top: 1px solid var(--line); color: var(--faint); font: 10px var(--mono); }
@keyframes beacon {
  0%,100% { opacity: .58; transform: scale(.90); }
  50% { opacity: 1; transform: scale(1); }
}
@media (max-width: 1180px) {
  .nav { grid-template-columns: repeat(4, 1fr); }
  .nav-button:nth-child(4n) { border-right: 0; }
...
@media (max-width: 760px) {
  ...
  .system-ribbon { grid-column: 1 / -1; grid-row: 2; justify-content: flex-start; margin-bottom: 12px; }
...
@media print {
  ...
  *, *::before, *::after { animation:none !important; box-shadow:none !important; }
}
</style>
</head>
<body>
<div class="app">
  <header class="masthead">
    ...
    <div class="head-tools">
      ...
      <button class="control" id="refreshBtn" aria-busy="false"><span><strong>刷新</strong><small>SYNC ACTIVE</small></span></button>
    </div>
  </header>
...
const VIEW_META = {
  ...
  memories:['记忆浏览','服务端筛选和分页，内容默认隐藏。列表不预载完整正文。','BOUNDED RECALL'],
...
function setControlFeedback(button, state, label, meta = '') {
  if (!button) return;
  const title = button.querySelector('strong');
  const detail = button.querySelector('small');
  if (!button.dataset.feedbackLabel) {
    button.dataset.feedbackLabel = title?.textContent || '';
    button.dataset.feedbackMeta = detail?.textContent || '';
    button.dataset.feedbackDisabled = button.disabled ? 'true' : 'false';
  }
...
function restoreControlFeedback(button) {
  if (!button || !button.dataset.feedbackLabel) return;
  const title = button.querySelector('strong');
  const detail = button.querySelector('small');
  button.classList.remove('is-loading', 'is-success', 'is-error');
  button.setAttribute('aria-busy', 'false');
  button.disabled = button.dataset.feedbackDisabled === 'true';
  if (title) title.textContent = button.dataset.feedbackLabel;
  if (detail) detail.textContent = button.dataset.feedbackMeta;
  delete button.dataset.feedbackLabel;
  delete button.dataset.feedbackMeta;
  delete button.dataset.feedbackDisabled;
}

async function withControlFeedback(button, labels, work) {
  if (!button) return work();
  clearTimeout(button._feedbackTimer);
  setControlFeedback(button, 'loading', labels.loading || '正在处理', labels.loadingMeta || 'LOCAL ACTION');
  try {
    const result = await work();
    setControlFeedback(button, 'success', labels.success || '已完成', labels.successMeta || 'DONE');
    button._feedbackTimer = setTimeout(() => restoreControlFeedback(button), 2800);
    return result;
  } catch (error) {
    setControlFeedback(button, 'error', labels.error || '执行失败', labels.errorMeta || 'RETRY');
    button._feedbackTimer = setTimeout(() => restoreControlFeedback(button), 3200);
    throw error;
  }
}
...
async function renderOverview() {
  ...
  $('viewRoot').innerHTML = `${viewHead('overview', `SNAPSHOT · ${esc(when(data.generated_at))}`)}
    <section class="hero"><div class="hero-main"><div class="micro">SYSTEM VERDICT</div><div class="verdict ${esc(data.status)}">${esc(data.status_label)}</div><p class="hero-copy">${esc(data.status === 'healthy' ? '关键证据均在有效期内。健康结论来自真实运行、调度器、结果与备份证明。' : '系统没有把未知或部分完成伪装成健康。请按下方证据和风险逐项检查。')}</p></div>
      <div class="hero-side">
...
async function renderMemories() {
  ...
  const rows = data.items.map(item => `
    <button class="row" data-memory-id="${esc(item.id)}">
      ...
      <p>${esc(item.reveal_hint || '正文默认隐藏。选择记录后可在本机确认显示。')}</p>
  ...
  $('viewRoot').innerHTML = `${viewHead('memories', `${esc((data.backend || 'unknown').toUpperCase())} · ${data.index_fresh ? 'INDEX FRESH' : 'INDEX LAG'}`)}
    ${indexNotice}
    <div class="toolbar">
      <input class="field" id="memoryQuery" value="${esc(q)}" placeholder="按关键词检索本机记忆">
...
async function renderActive({quiet = false, throwOnError = false} = {}) {
...
async function openMemory(id) {
  ...
  showDrawer(`记忆 · ${id}`, `${detailBlocks(data)}<div><button class="control" data-memory-reveal="${esc(id)}"><span><strong>显示本机正文</strong><small>CONFIRM</small></span></button></div>`);
  bindDrawerEvents();
}

async function revealMemory(id, control) {
  confirmAction({
    title:'确认显示本机正文',
    copy:'正文信息会从本机记录中读取，只用于当前会话，不会写入日志。',
    impact:'读取后将在抽屉内直接展示正文内容。',
    control,
    execute: async () => {
      const data = await api(`/api/v1/memories/${encodeURIComponent(id)}?reveal=1`);
      showDrawer(`记忆正文 · ${id}`, detailBlocks(data));
      bindDrawerEvents();
    }
  });
}
...
function requestJobAction(kind, control) {
...
function bindDrawerEvents() {
  ...
  document.querySelectorAll('[data-memory-reveal]').forEach(button => button.onclick = (event) => {
    event.preventDefault();
    event.stopPropagation();
    revealMemory(button.dataset.memoryReveal, button).catch(error => toast(error.message, true));
  });
}
...
function bindViewEvents() {
  ...
  $('profileSearch')?.addEventListener('click', event => withControlFeedback(event.currentTarget, {loading:'正在查询', loadingMeta:'SERVER FILTER', success:'结果已更新', successMeta:'REFRESHED'}, async () => { profileOffset = 0; await renderProfile(); bindViewEvents(); }).catch(error => toast(error.message, true)));
  $('profilePrev')?.addEventListener('click', event => withControlFeedback(event.currentTarget, {loading:'正在读取', loadingMeta:'PREVIOUS', success:'列表已更新', successMeta:'REFRESHED'}, async () => { profileOffset = Math.max(0, profileOffset - 20); await renderProfile(); bindViewEvents(); }).catch(error => toast(error.message, true)));
  $('profileNext')?.addEventListener('click', event => withControlFeedback(event.currentTarget, {loading:'正在读取', loadingMeta:'NEXT PAGE', success:'列表已更新', successMeta:'REFRESHED'}, async () => { profileOffset += 20; await renderProfile(); bindViewEvents(); }).catch(error => toast(error.message, true)));
  document.querySelector('[data-profile-merge]')?.addEventListener('click', event => confirmAction({
    title:'合并已选画像',
...
$('refreshBtn').addEventListener('click', event => withControlFeedback(event.currentTarget, {
  loading:'正在刷新', loadingMeta:'LIVE READ', success:'已刷新', successMeta:'UP TO DATE'
}, async () => {
  await renderActive({throwOnError:true});
  toast('当前模块已刷新');
