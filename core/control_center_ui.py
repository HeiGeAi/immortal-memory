...
async function renderAgent() {
  ...
  $('viewRoot').innerHTML = `${viewHead('agent', `ENTRY · ${data.entry.exists ? 'READY' : 'MISSING'}`)}
    <section class="grid grid-2">
      ...
      <form class="card" id="contextForm"><div class="card-top"><span class="mono">CONTEXT COMPILER</span>${tag('ready')}</div><h4>生成任务上下文</h4><p>只接受目标和固定模式，不接受命令字符串。</p><input class="field" id="contextGoal" required maxlength="160" placeholder="例如：为发布 v1.0.0 做最终审查"><select class="field" id="contextMode"><option value="auto">自动</option><option value="advisor">决策顾问</option><option value="writer">写稿审稿</option><option value="reviewer">复核审阅</option><option value="business">商业判断</option><option value="project">项目推进</option><option value="custom">自定义</option></select><button class="control primary" type="submit"><span><strong>生成上下文</strong><small>CONTROLLED JOB</small></span></button></form>
    </section>
